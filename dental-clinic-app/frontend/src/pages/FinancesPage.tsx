...
const FinancesPage: React.FC = () => {
  ...
  return (
    <div className="p-8">
      <div className="mb-6">
        <h1 className="text-xl font-semibold text-surface-900">Finances</h1>
        <p className="mt-1 text-sm text-surface-500">Manage payments and expenses</p>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {activeTab === 'payments' ? <PaymentTable /> : <ExpenseTable token={token ?? ''} />}
        </motion.div>
      </AnimatePresence>
    </div>
  );
