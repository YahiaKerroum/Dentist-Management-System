...
7. **Identity depth pass** (2026-07-08, follows the brand pass below): (a) every page's `<h1>` now uses `font-display` (Space Grotesk) — Patients/Appointments/Finances/Treatments/Staff/Reports/Profile/PatientDetail, plus the Treatments stat numerals — matching the dashboard hero; (b) the Patients loading state is a real branded skeleton of the page layout (header + controls + row cards) instead of a bare centered spinner; (c) `ui/EmptyState.tsx` now carries the ECG-line motif (a flat pulse that blips once through the icon); (d) **Reports chart system unified**: new `src/lib/chartTheme.ts` is the single source for chart color/typography — a CVD-validated, brand-teal-led 6-slot categorical palette (`categoricalColor()` folds the 7th+ series onto a neutral "Other", never cycles hues), reserved status colors mapped to the app's semantic tokens (positive/neutral/attention/negative), one shared `chartTooltip`, grid, and axis-tick style. All 18 analytics cards refactored onto it: the generic rainbow `#3B82F6…` arrays and per-file `#e5e7eb` tooltip borders are gone, the appointment-heatmap sequential ramp is now brand-teal (was blue), status badges use semantic tokens, card headers use `font-display`, and every numeral is `tabular-nums`. Palette validated via the dataviz skill's `validate_palette.js` (all six checks pass on the white chart surface). This completes the "Reports still looks pre-redesign" item flagged at the end of the brand pass.
